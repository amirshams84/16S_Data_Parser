...
def statistics_table_function(Excel_File_PATH):
	# ++++++++++++++++++++++++++++++ LOAD ALL DATA
	Biom_DICT, Biom_LIST = excel_to_vertical_dict_converter(Excel_File_PATH, 'Biome_Data')
	Sample_metadata_DICT, Sample_metadata_LIST = excel_to_vertical_dict_converter(Excel_File_PATH, 'Sample_metadata')
	OTU_metadata_DICT, OTU_metadata_LIST = excel_to_vertical_dict_converter(Excel_File_PATH, 'OTU_metadata')
	Sample_metadata_MATRIX, Sample_metadata_MATRIX_LIST = excel_to_matrix_converter(Excel_File_PATH, 'Sample_metadata', index_col=0)
	# ------------------------------ END OF LOADING
	
	statistics_table_excel_DICT = {}
	statistics_table_excel_LIST = []
	###############################################################################
	statistics_table_header_LIST = []
	statistics_table_header_LIST.extend(Sample_metadata_LIST)
	statistics_table_header_LIST.extend(['Total_Number', 'Classified_Number', 'Unclassified_Number', 'Classified_Percentage', 'Unclassified_Percentage'])
	statistics_table_excel_LIST.extend(statistics_table_header_LIST)
	for each_Label in statistics_table_excel_LIST:
		statistics_table_excel_DICT[each_Label] = []
	#print statistics_table_excel_DICT
	#print statistics_table_excel_LIST
	#sys.exit(2)
	#print statistics_table_excel_LIST
	#print statistics_table_excel_DICT
	...
	table_body_string = ''
	table_body_string += '\t<tbody>\n'
	for each_Sample in Sample_metadata_DICT['Sample_ID']:
		#statistics_table_excel_DICT['Sample_ID'].append(each_Sample)
		total_read_count = sum(map(int, Biom_DICT[each_Sample]))
		if Unclassified_Flag is True:
			unclassified_read_count = map(int, Biom_DICT[each_Sample][Unclassified_index])
		else:
			unclassified_read_count = 0
		classified_read_count = int(total_read_count - unclassified_read_count)
		table_body_string += '\t\t<tr>\n'
		for each_SAMPLE_metadata in Sample_metadata_MATRIX_LIST:
			#print each_SAMPLE_metadata
			table_body_string += '\t\t\t<td class="text-center">' + str(Sample_metadata_MATRIX[each_Sample][each_SAMPLE_metadata]) + '</td>\n'
			statistics_table_excel_DICT[each_SAMPLE_metadata].append(Sample_metadata_MATRIX[each_Sample][each_SAMPLE_metadata])
		#table_body_string += '\t\t\t<td class="text-center">' + str(each_sample) + '</td>\n'
		table_body_string += '\t\t\t<td class="text-center">' + str(total_read_count) + '</td>\n'
		statistics_table_excel_DICT['Total_Number'].append(total_read_count)
		table_body_string += '\t\t\t<td class="text-center">' + str(classified_read_count) + '</td>\n'
		statistics_table_excel_DICT['Classified_Number'].append(classified_read_count)
		table_body_string += '\t\t\t<td class="text-center">' + str(unclassified_read_count) + '</td>\n'
		statistics_table_excel_DICT['Unclassified_Number'].append(unclassified_read_count)
		table_body_string += '\t\t\t<td class="text-center">' + str(round_float(percentage(classified_read_count, total_read_count))) + '</td>\n'
		statistics_table_excel_DICT['Classified_Percentage'].append(str(round_float(percentage(classified_read_count, total_read_count))))
		table_body_string += '\t\t\t<td class="text-center">' + str(round_float(percentage(unclassified_read_count, total_read_count))) + '</td>\n'
		statistics_table_excel_DICT['Unclassified_Percentage'].append(str(round_float(percentage(unclassified_read_count, total_read_count))))
		table_body_string += '\t\t</tr>\n'
		
	table_body_string += '\t</tbody>\n'
	###############################################################################
	#Write to Excel
	flag = any_dict_to_excel_file_append(Excel_File_PATH, 'Statistics', statistics_table_excel_DICT, statistics_table_excel_LIST)
	if flag is True:
		pass
	###############################################################################
	statistics_table_html_string = """
...
def statistics_table_function_oldi(EXCEL_file_PATH):
	#LOADING DICTs
	...
	if '0' in OTU_ID_lower_DICT:
		unclassified_index = OTU_ID_lower_DICT.index('0')
		unclassified_otu_ALIAS = OTU_metadata_vertical_DICT['OTU_ALIAS'][unclassified_index]
		print unclassified_otu_ALIAS
		no_unclassified_flag = False
	else:
		no_unclassified_flag = True
	...
	for each_sample in STRAIN_data_horizontal_LIST:
		each_sample_index = STRAIN_data_horizontal_LIST.index(each_sample)
		OTU_value_list = map(int, STRAIN_data_horizontal_DICT[each_sample][3:])
		total_read_count = sum(OTU_value_list)
		if no_unclassified_flag is False:
			unclassified_read_count = int(STRAIN_data_vertical_DICT[unclassified_otu_ALIAS][each_sample_index])
		else:
			unclassified_read_count = 0
		classified_read_count = int(total_read_count - unclassified_read_count)
...
def rarefaction_curve_plot(Excel_File_PATH, mothur_exec_path, processors, outputdir_PATH):
	...
	##################################################################################################################
	Shared_vertical_DICT, Shared_vertical_LIST = any_file_to_dict_converter_vertical(Shared_File_PATH)
...
def biom_to_excel_converter_oldi(biom_file_PATH, Design_file_PATH, EXCEL_file_PATH):
	
	#Biom Parsing
	biom_table_HANDLE = biom.load_table(biom_file_PATH)
	
	#DEFINE OUTPUTS
	Biome_data_DataFrame_DICT = {}
	Biome_data_DataFrame_LIST = []
	#Step1: FILLOUT shared_file_DataFrame_DICT
	Biome_data_DataFrame_LIST = ['Groups', 'numOtus']
	#Biome_data_DataFrame_DICT['label'] = []
	Biome_data_DataFrame_DICT['Groups'] = []
	Biome_data_DataFrame_DICT['numOtus'] = []
	...
	for each_sample_ID in sample_ID_LIST:
		...
		Biome_data_DataFrame_DICT['Groups'].append(sample_alias_name)
		#Biome_data_DataFrame_DICT['label'].append('BIOM_SLICER')
		Biome_data_DataFrame_DICT['numOtus'].append(numOtus_value)
		alias_counter += 1
	...
	for each_OTU_ID in OTU_ID_LIST:
		...
		Biome_data_DataFrame_DICT[OTU_alias_name] = map(int, each_OTU_ID_data_LIST)
		Biome_data_DataFrame_LIST.append(OTU_alias_name)
		alias_counter += 1
	
	Biome_DataFrame = pandas.DataFrame.from_dict(Biome_data_DataFrame_DICT)
	write_Pandas_DataFrame_to_Excel(Biome_DataFrame, EXCEL_file_PATH, 'Biome', Biome_data_DataFrame_LIST)
	sys.exit(2)
	#Excel_writer = pandas.ExcelWriter(EXCEL_file_PATH, engine='openpyxl')
	#Biome_DataFrame.to_excel(Excel_writer, sheet_name='Biome', columns=Biome_data_DataFrame_LIST, index=False, verbose=True)
	#Biome_DATAFrame.to_csv(EXCEL_file_PATH, sep='\t', columns=Biome_data_DataFrame_LIST, index=False)
	write_huge_DataFrame_to_Excel(Biome_DataFrame, EXCEL_file_PATH)
	#Excel_writer.save()
...
def biom_to_excel_converter(Biom_File_PATH, Design_File_PATH, Excel_File_PATH):
	...
	Biom_DICT = {}
	Biom_LIST = []
	Sample_ID_LIST = map(str, Biom_HANDLE.ids(axis='sample'))
	Biom_ID_LIST = map(str, Biom_HANDLE.ids(axis='observation'))
	Biom_DICT['OTU_ID'] = []
	Biom_DICT['Biom_ID'] = []
	Biom_LIST = ['OTU_ID']
	...
	for each_Sample in Sample_ID_LIST:
		Biom_DICT[each_Sample] = map(int, Biom_HANDLE.data(each_Sample, axis='sample').tolist())
	...
	Pandas_DataFrame = pandas.DataFrame.from_dict(Biom_DICT)
	Pandas_DataFrame.to_excel(Excel_writer, sheet_name='Biome_Data', columns=Biom_LIST, header=True, index=False)
	# ######################################################################
	# Sample_metadata
	# ######################################################################
	...
	if Biom_HANDLE.metadata(axis='sample') is not None:
		...
		for each_Sample in Sample_ID_LIST:
			Sample_DICT['Sample_ID'].append(each_Sample)
		...
		Pandas_DataFrame = pandas.DataFrame.from_dict(Sample_DICT)
		Pandas_DataFrame.to_excel(Excel_writer, sheet_name='Sample_metadata', columns=Sample_LIST, header=True, index=False)
	else:
		Sample_DICT = {}
		Sample_LIST = ['Sample_ID', 'Label']
		Sample_DICT['Sample_ID'] = []
		Sample_DICT['Label'] = []
		for each_Sample in Sample_ID_LIST:
			Sample_DICT['Sample_ID'].append(each_Sample)
	...
	if Biom_HANDLE.metadata(axis='observation') is not None:
		...
		Pandas_DataFrame = pandas.DataFrame.from_dict(OTU_DICT)
		Pandas_DataFrame.to_excel(Excel_writer, sheet_name='OTU_metadata', columns=OTU_LIST, header=True, index=False)
		OTU_MATRIX = {}
		OTU_MATRIX_LIST = []
	...
	if Design_File_PATH is not None:
		Design_DICT = {}
		Design_LIST = []
		Design_DICT, Design_LIST = any_file_to_dict_converter_vertical(Design_File_PATH)

		key_column = ''
		key_column_len = 0
		#print SAMPLE_metadata_DATAFrame_DICT['SAMPLE_ID']
		for each_Column in Design_LIST:
			match_rank = len(match_two_list(Sample_DICT['Sample_ID'], Design_DICT[each_Column]))
			if match_rank > key_column_len:
				key_column_len = match_rank
				key_column = each_Column
		if key_column_len > 1:
			...
			Pandas_DataFrame = pandas.DataFrame.from_dict(Category_DICT)
			Pandas_DataFrame.to_excel(Excel_writer, sheet_name='Design', columns=Category_LIST, header=True, index=False)
		else:
	...
	Pandas_DataFrame = pandas.DataFrame.from_dict(TAX_DICT)
	Pandas_DataFrame.to_excel(Excel_writer, sheet_name='TAX_metadata', columns=TAX_LIST, header=True, index=False)
	#print TAX_MATRIX
	...
	for each_lineage in lineage_Name_LIST:
		...
		for each_TAX_ID in TAX_map_DICT[each_lineage]:
			Lineage_DICT['TAX_ID'].append(TAX_MATRIX[each_TAX_ID])
		...
		for each_Sample in Biom_MATRIX_LIST:
			...
			for each_TAX_ID in TAX_map_DICT[each_lineage]:
				...
				Lineage_DICT[each_Sample].append(sum(map(int, list_of_value)))
		
		Pandas_DataFrame = pandas.DataFrame.from_dict(Lineage_DICT)
		Pandas_DataFrame.to_excel(Excel_writer, sheet_name=each_lineage, columns=Lineage_LIST, header=True, index=False)
	Excel_writer.save()
	#Emptying
	Biom_DICT = {}
	Lineage_DICT = {}
...
def remove_ambiguity_from_taxonomy(taxonomy_LIST):
	...
	for each_tax in taxonomy_LIST:
		...
		if each_tax in ['root', 'unknown', 'domain', None, '', 'k_', 'p_', 'c_', 'o_', 'f_', 'g_', 's_']:
			continue
		else:
...
def taxonomy_interpret_factory_old(Excel_file_PATH, Design_Sample_LIST, Design_OTU_LIST):
...
def taxonomy_interpret_factory(Excel_file_PATH):
	#STEP1: ACCESSING THE EXCEL FILE
	EXCEL_writer = pandas.ExcelWriter(Excel_file_PATH, engine='openpyxl')
	OpenBook = openpyxl.load_workbook(Excel_file_PATH)
	EXCEL_writer.book = OpenBook

	#Step2: Open Biome file
	OTU_table_vertical_DICT, OTU_table_vertical_LIST = excel_to_vertical_dict_converter(Excel_file_PATH, 'OTU_table')

	#Step3: Creating TAX_metadata sheet
	OTU_metadata_MATRIX, OTU_metadata_LIST = excel_to_matrix_converter(Excel_file_PATH, 'OTU_metadata', 0)
	...
	TAX_metadata_DataFrame_DICT['TAX_LEVEL'] = []
	#TAX_metadata_DataFrame_DICT['TAX_ID'] = []
	TAX_metadata_DataFrame_DICT['TAXONOMY'] = []
	TAX_metadata_DATAFrame_header_LIST = ['TAX_ALIAS', 'TAX_LEVEL', 'TAXONOMY']
	
	alias_counter = 1
	OTU_name_LIST = []
	OTU_name_LIST = OTU_table_vertical_LIST[3:]
	OTU_map_DICT = {}
	for each_OTU in OTU_name_LIST:
		TAX_alias_name = 'TAX_ALIAS_' + str(alias_counter).zfill(6)
		TAX_metadata_DataFrame_DICT['TAX_ALIAS'].append(TAX_alias_name)
		TAX_metadata_DataFrame_DICT['TAX_LEVEL'].append('OTU')
		#TAX_metadata_DataFrame_DICT['TAX_ID'].append(OTU_metadata_MATRIX[each_OTU]['OTU_ID'])
		if 'taxonomy' in OTU_metadata_LIST:
			TAX_metadata_DataFrame_DICT['TAXONOMY'].append(OTU_metadata_MATRIX[each_OTU]['taxonomy'])
		else:
			TAX_metadata_DataFrame_DICT['TAXONOMY'].append(OTU_metadata_MATRIX[each_OTU]['OTU_ID'])
		OTU_map_DICT[each_OTU] = TAX_alias_name
		alias_counter += 1
	#TAX_metadata_data_frame = pandas.DataFrame.from_dict(TAX_metadata_DataFrame_DICT)
	#TAX_metadata_data_frame.to_excel(EXCEL_writer, sheet_name='TAX_metadata', columns=TAX_metadata_DATAFrame_header_LIST, index=False, engine='xlsxwriter')
	...
	OTU_data_DICT = {}
	OTU_data_LIST = []
	OTU_data_LIST = ['label', 'Groups', 'numOtus']
	
	OTU_data_DICT['label'] = OTU_table_vertical_DICT['label']
	OTU_data_DICT['Groups'] = OTU_table_vertical_DICT['Groups']
	OTU_data_DICT['numOtus'] = [len(OTU_name_LIST)] * len(OTU_table_vertical_DICT['Groups'])
	for each_OTU in OTU_name_LIST:
		OTU_data_DICT[OTU_map_DICT[each_OTU]] = OTU_table_vertical_DICT[each_OTU]
		OTU_data_LIST.append(OTU_map_DICT[each_OTU])
	
	OTU_data_DICT_data_frame = pandas.DataFrame.from_dict(OTU_data_DICT)
	OTU_data_DICT_data_frame.to_excel(EXCEL_writer, sheet_name='OTU', columns=OTU_data_LIST, index=False, engine='xlsxwriter')
	
	# ##################################################
	# ##################################################
	# Step 4: Creating taxonomy level sheets
	#STEP2: FILL OUT MULTI_SHARED SHARED FILE
	lineage_Name_LIST = ['Kingdom', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species']
	lineage_target_DICT = {}
	lineage_target_DICT['Kingdom'] = []
	...
	#STEP1: Reduce the number of similar taxonomy
	unique_taxonomy_LIST = []
	unique_taxonomy_LIST = list(set(TAX_metadata_DataFrame_DICT['TAXONOMY']))
	#print unique_taxonomy_LIST
	#'k_bacteria;p_bacteroidetes;c_bacteroidia;o_bacteroidales;f_bacteroidaceae;*f_bacteroidaceae;*f_bacteroidaceae;', 'k_bacteria;p_tenericutes;c_mollicutes;o_rf39;*o_rf39;*o_rf39;*o_rf39;', 'k_bacteria;p_firmicutes;c_clostridia;o_clostridiales;f_ruminococcaceae;*f_ruminococcaceae;*f_ruminococcaceae;', 
	#'k_bacteria;p_firmicutes;c_clostridia;o_clostridiales;f_peptococcaceae;*f_peptococcaceae;*f_peptococcaceae;',
	for each_taxonomy in unique_taxonomy_LIST:
	...
	TAXIC_metadata_DataFrame_DICT['TAX_ALIAS'] = []
	TAXIC_metadata_DataFrame_DICT['TAX_LEVEL'] = []
	TAXIC_metadata_DataFrame_DICT['TAXONOMY'] = []
	...
	for each_lineage in lineage_Name_LIST:
		...
		for each_taxonomy in lineage_target_DICT[each_lineage]:
			...
			TAXIC_metadata_DataFrame_DICT['TAX_ALIAS'].append(TAX_alias_name)
			TAXIC_metadata_DataFrame_DICT['TAX_LEVEL'].append(each_lineage)
			TAXIC_metadata_DataFrame_DICT['TAXONOMY'].append(each_taxonomy)
	...
	for each_OTU_ALIAS, each_taxonomy in itertools.izip(TAX_metadata_DataFrame_DICT['TAX_ALIAS'], TAX_metadata_DataFrame_DICT['TAXONOMY']):
	...
	for each_lineage in lineage_Name_LIST:
		OTU_name_LIST = ['label', 'Groups', 'numOtus']
		MULTI_TAX_SHARED_DICT[each_lineage] = {}
		MULTI_TAX_SHARED_DICT[each_lineage]['label'] = OTU_data_DICT['label']
		MULTI_TAX_SHARED_DICT[each_lineage]['Groups'] = OTU_data_DICT['Groups']
		MULTI_TAX_SHARED_DICT[each_lineage]['numOtus'] = [len(TAX_map_DICT[each_lineage])] * len(OTU_data_DICT['Groups'])
		OTU_name_LIST.extend(TAX_map_DICT[each_lineage])
		for each_TAX_alias in TAX_map_DICT[each_lineage]:
			MULTI_TAX_SHARED_DICT[each_lineage][each_TAX_alias] = []
			list_of_values = []
			for each_OTU_ALIAS in TAX_map_DICT[each_lineage][each_TAX_alias]:
					list_of_values.append(map(int, OTU_data_DICT[each_OTU_ALIAS]))
			MULTI_TAX_SHARED_DICT[each_lineage][each_TAX_alias] = [sum(x) for x in zip(*list_of_values)]
		lineage_data_frame = pandas.DataFrame.from_dict(MULTI_TAX_SHARED_DICT[each_lineage])
		lineage_data_frame.to_excel(EXCEL_writer, sheet_name=each_lineage, columns=OTU_name_LIST, index=False, engine='xlsxwriter')
	#print MULTI_TAX_SHARED_DICT['Phylum']
	...
	TAX_metadata_DataFrame_DICT['TAX_ALIAS'].extend(TAXIC_metadata_DataFrame_DICT['TAX_ALIAS'])
	TAX_metadata_DataFrame_DICT['TAX_LEVEL'].extend(TAXIC_metadata_DataFrame_DICT['TAX_LEVEL'])
	TAX_metadata_DataFrame_DICT['TAXONOMY'].extend(TAXIC_metadata_DataFrame_DICT['TAXONOMY'])
...
def any_dict_to_excel_file_append(Excel_File_PATH, sheetname, any_Dict, any_LIST=None):
	...
	any_DICT_DataFrame.to_excel(Excel_writer, sheet_name=sheetname, columns=any_LIST, index=False, engine='xlsxwriter')
	Excel_writer.save()
	return True


def excel_to_shared_file_converter(Excel_File_PATH, outputdir_PATH):
	OTU_vertical_DICT, OTU_vertical_LIST = excel_to_vertical_dict_converter(Excel_File_PATH, 'Biome_Data')
	OTU_horizontal_DICT, OTU_horizontal_LIST = excel_to_horizontal_dict_converter(Excel_File_PATH, 'Biome_Data', index_col=0)
	Sample_metadata_DICT, Sample_metadata_LIST = excel_to_vertical_dict_converter(Excel_File_PATH, 'Sample_metadata')
	OTU_metadata_DICT, OTU_metadata_LIST = excel_to_vertical_dict_converter(Excel_File_PATH, 'OTU_metadata')
	Shared_DICT = {}
	...
	Shared_DICT['Groups'] = []
	Shared_DICT['numOtus'] = []
	Shared_LIST.extend(OTU_metadata_DICT['OTU_ID'])
	for each_OTU in OTU_metadata_DICT['OTU_ID']:
		Shared_DICT[each_OTU] = []
	NumOtus_value = str(len(OTU_metadata_DICT['OTU_ID']))

	for each_Sample in Sample_metadata_DICT['Sample_ID']:
		Shared_DICT['label'].append('BiomSlicer')
		Shared_DICT['Groups'].append(each_Sample)
		Shared_DICT['numOtus'].append(NumOtus_value)
	for each_OTU in OTU_horizontal_LIST:
		Shared_DICT[each_OTU] = map(str, OTU_horizontal_DICT[each_OTU][3:])
	Shared_File_PATH = outputdir_PATH + 'Shared_file.txt'
	any_dict_to_text_converter(Shared_DICT, Shared_LIST, Shared_File_PATH)
...
def main(argv):
	...
	plotly_rarefaction_html_string, plotly_rarefaction_javascript_string = rarefaction_curve_plot(Excel_File_PATH, mothur_exec_PATH, args.processors, args.outputdir)
