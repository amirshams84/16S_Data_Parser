...
def main(argv):
	...
	main_file.add_argument("--shared", help="Mothur microbiome abundance matrix file(Shared file: https://www.mothur.org/wiki/Shared_file)", action='store')
	
	main_file.add_argument("--biom", help="Universal microbiome abundance matrix file(http://biom-format.org/documentation/format_versions/biom-1.0.html)", action='store')
	
	main_file.add_argument("--phylotype", help="Mothur microbiome abundance matrix file including summarized taxonomic information", action='store')
	...
	main_file.add_argument("--taxlevel", help="Desired taxonomy level['phylum', 'order', 'class', 'family', 'genus', 'species']", action='store')

	# #################### GENERAL PARAMETERS
	general = parser.add_argument_group('general parameters')
	
	general.add_argument("--processors", help="Number of CPU cores to be used", action='store')
	...
	report_string += "######################################################################################################################################\n"
	print "######################################################################################################################################"
	report_string += "MICROBIOME SLICER 1.0 EXECUTION HAS INITIATED" + '\n'
	print "MICROBIOME SLICER 1.0 EXECUTION HAS INITIATED"
	...
	if isPathExist(DEFAULT_OUTPUTDIR) is False:
		flag = create_folder(DEFAULT_OUTPUTDIR)
		if flag is False:
			print "Can not create folder at specified path: ", DEFAULT_OUTPUTDIR
			print "ABORTING!!!"
			sys.exit(2)
	...
	global report_file
	report_file = args.outputdir + "microbiome_slicer_report.txt"
	...
	if isPathExist(DEFAULT_EXECDIR) is False:
		flag = create_folder(DEFAULT_EXECDIR)
		if flag is False:
			print "Can not create folder at specified path: ", DEFAULT_EXECDIR
			print "ABORTING!!!"
			sys.exit(2)
	args.execdir = DEFAULT_EXECDIR
	print "[--execdir]: Executive directory is: ", args.execdir
	report("[--execdir]: Executive directory is: " + args.execdir)
	# ###########################################################################

	# ########### EXECUTABLES CHECKING
	print "\n###################################################################"
	report("\n###################################################################")
	...
	if args.shared is not None:
		if isFileExist(args.shared) is False:
			...
			shared_file_VALIDATED_PATH = args.outputdir + args.prefix + '_VALIDATED_shared_file_STEP1.txt'
			flag = shared_file_validator(args.shared, args.prefix, shared_file_VALIDATED_PATH)
			if flag is True:
				args.shared = shared_file_VALIDATED_PATH
	# ###########################################################################
	...
	# ########### BIOM FILE PROCESSING
		...
		if isFileExist(args.biom) is False:
			...
			error("ABORTING!!!")
			sys.exit(2)
		biom_to_shared_PATH = args.outputdir + args.prefix + '_BIOM_CONVERTED_shared_file_STEP1.txt'
		flag = biom_to_shared_convert(biom_to_shared_PATH, args.biom, mothur_exec_PATH, args.processors, args.outputdir)
		if flag is False:
			print "Some this is not right during conversion"
		shared_file_VALIDATED_PATH = args.outputdir + args.prefix + '_VALIDATED_shared_file_STEP1.txt'
		flag = shared_file_validator(biom_to_shared_PATH, args.prefix, shared_file_VALIDATED_PATH)
		if flag is True:
			args.shared = shared_file_VALIDATED_PATH
	# ###########################################################################

	# ########### PHYLOTYPE FILE PROCESSING
	elif args.phylotype is not None:
		...
		shared_file_PATH = args.outputdir + args.prefix + '_shared_file_from_phylotype.txt'
		flag = phylotype_to_shared_convert(args.phylotype, shared_file_PATH, args.taxlevel, args.prefix, args.outputdir)
		if flag is True:
			pass
		shared_file_VALIDATED_PATH = args.outputdir + args.prefix + '_VALIDATED_shared_file_STEP1.txt'
		flag = shared_file_validator(shared_file_PATH, args.prefix, shared_file_VALIDATED_PATH)
		if flag is True:
			args.shared = shared_file_VALIDATED_PATH
	# ###########################################################################

	# ########### LOAD THE DEFAULT INPUT FILES
	elif args.shared is None and args.biom is None and args.phylotype is None:
	...
	report("[--shared]: Validated shared file is: " + args.shared)
	print "[--shared]: Validated shared file is: ", args.shared
	print "# ####################################################################################"
	# ###########################################################################
	...
	report("[--design]: Validated design file is: " + args.design)
	print "[--design]: Validated design file is: ", args.design
	print "# ####################################################################################"
	# ###########################################################################

	# ##################### STEP2: MATCH SHARED FILE BASED ON DESIGN FILE
	...
	flag = design_to_control_convert(args.design, control_from_design_file_PATH)
	if flag is True:
		pass
	...
	report("[--shared]: Matched shared file is: " + args.shared)
	print "[--shared]: Matched shared file is: ", args.shared
	report("[--design]: Matched design file is: " + args.design)
	print "[--design]: Matched design file is: ", args.design
	print "Matching shared file with design file PASSED!!!"
	report("Matching shared file with design file PASSED!!!")
	print "# ####################################################################################"
	# ##############################################################################################################################################################
	
	# ##################### STEP3: BRIEF STATISTICS TABLE
	brief_statistics_table_html_string = ''
	brief_statistics_table_javascript_string = ''
	brief_statistics_table_html_string = brief_statistics_table(args.shared, args.design)
	# ##############################################################################################################################################################
	...
	# ##################### STEP4: ALPHA DIVERSITY INDEX BOXPLOT
	...
	method_NAME = 'shannon'
	collector_curve_SHANNON_html_string, collector_curve_SHANNON_javascript_string = plotly_Collector_Curve_SCATTERPLOT(args.shared, args.design, mothur_exec_PATH, args.prefix, frequency_VALUE, method_NAME, args.processors, args.outputdir)
	...
	if flag is True:
		...
		sys.exit(2)
	# ##################### STEP7: Bacterial_RELATIVE ABUNDANCE
	...
	"""
	html_file_string += brief_statistics_table_html_string
	html_file_string += plotly_natural_abundance_barplot_html_string
	html_file_string += plotly_rarefaction_html_string
	#html_file_string += plotly_alpha_diversity_boxplot_html_string
	#html_file_string += collector_curve_SHANNON_html_string
	
	
	
	
	html_file_string += plotly_Bacterial_relative_abundance_Barplot_html_string
	...
	html_file_string += """
						...
						<script type="text/javascript">
	"""
	html_file_string += brief_statistics_table_javascript_string
	html_file_string += plotly_natural_abundance_barplot_javascript_string
	html_file_string += plotly_rarefaction_javascript_string
	#html_file_string += plotly_alpha_diversity_boxplot_javascript_string
	#html_file_string += collector_curve_SHANNON_javascript_string
	
	
	
	html_file_string += plotly_Bacterial_relative_abundance_Barplot_javascript_string
	...
	print "MICROBIOME SLICER EXECUTION COMPLETED."
	report("MICROBIOME SLICER EXECUTION COMPLETED.")
...
def brief_statistics_table(shared_file_PATH, design_file_PATH):
	headers_list = []
	columns_dict = {}
	...
	for each_head in headers_list:
		if each_head.lower() in ['label', 'numotus', 'group', 'groups']:
			continue
		else:
...
def plotly_Dominant_Entities_Distribution_Boxplot_DROPDOWN(shared_file, design_file):
	# ######################## PARSING DATA
	headers_list = []
	columns_dict = {}
	headers_list, columns_dict = mothur_shared_parser(shared_file)
	...
	for each_head in headers_list:
		if each_head.lower() in ['label', 'group', 'numotus']:
			...
			OTU_name_list.append(each_head)
	visibility_list_length = len(OTU_name_list) * len(reverse_design_dict.keys())
	alpha_visibility_list = []
	# ####################### TO GENERATE SWITCHING STRING
	VISIBLE_flag_list = []
	visibility_flag_dict = {}
	...
	for i in xrange(0, visibility_list_length, len(reverse_design_dict.keys())):
		...
		visible_index += 1
	# ######################## PLOTLY DATA CREATE
	Dominant_Entities_Distribution_boxplot_TRACE_list = []
	Dominant_Entities_Distribution_boxplot_LAYOUT = {}
	...
	for each_OTU in OTU_name_list:
		...
		for each_design in reverse_design_dict:
			...
			for each_sample in sample_list:
				each_sample_index = columns_dict['Group'].index(each_sample)
				OTU_value_list.append(columns_dict[each_OTU][each_sample_index])
			Dominant_Entities_Distribution_boxplot_TRACE_list.append(PLOTLY_GO.Box(
				x=each_design,
				y=OTU_value_list,
				name=each_design,
				hoverinfo='y+name',
				boxmean=True,
				visible=visibility_flag_dict[OTU_name_list.index(each_OTU)][reverse_design_dict.keys().index(each_design) * OTU_name_list.index(each_OTU)],
				legendgroup=each_OTU
				#orientation='h',
			)
			)
		VISIBLE_flag_list.append(
			dict(
				args=[
					dict(
						#title=title_dictionary[each_index],
						visible=visibility_flag_dict[OTU_name_list.index(each_OTU)]
					)
				],
				label=each_OTU,
				method='restyle',
	...
	Dominant_Entities_Distribution_boxplot_LAYOUT = PLOTLY_GO.Layout(
		boxmode='group',
		height=700,
		autosize=True,
		showlegend=False,
		title='Dominant entities distributions',
		hovermode='closest',
		titlefont=dict(
			family='Avenir',
			size=12
	...
	Dominant_Entities_Distribution_boxplot_FIGURE_objects_dict['dominant_entities'] = PLOTLY_GO.Figure(data=Dominant_Entities_Distribution_boxplot_TRACE_list, layout=Dominant_Entities_Distribution_boxplot_LAYOUT)
	# ###################################  PLOTLY PLOT CREATE ###############################
...
def plotly_natural_abundance_barplot(shared_file, design_file):
	...
	figure = plotly.tools.make_subplots(rows=1, cols=matrix_dimension, subplot_titles=subplot_titles_list, shared_xaxes=True, shared_yaxes=True)

	cartesian_length = range(1, matrix_dimension + 1)
	cartesian_dimension_list = []
	for each_cartesian in cartesian_length:
		cartesian_dimension_list.append((1, each_cartesian))
	...
	cartesian_count = 0
	for each_design in reverse_design_dict:
		for each_OTU in OTU_name_list[:10]:
...
def plotly_Collector_Curve_SCATTERPLOT(shared_file_PATH, design_file_PATH, mothur_exec_PATH, prefix_NAME, frequency_VALUE, method_NAME, processors_COUNT, outputdir_PATH):
	
	flag, stderr = execute_functions(mothur_collect_single, processors_COUNT, outputdir_PATH, 'multi', 'mothur', mothur_exec_PATH, shared_file_PATH, frequency_VALUE, method_NAME)
	if flag is False:
		print "Execution of mothur_collect_single failed!!!"
		sys.exit(2)
	else:
		extension_list = []
		extension_list.append(method_NAME)
		scanned_container = []
		flag = scandirs(outputdir_PATH, scanned_container, extension_list, mode='ex_partial')
		print "Scanning.."
		if flag is False:
			print "Failed :("
			print "This extension is not available: ", extension_list
	...
	collect_single_VALUE_dict = {}
	collect_single_HEADER_list = []
	for each_file in scanned_container:
		path, absname, ext = split_file_name(each_file)
		#print ext.split('.')
		sample_NAME = ext.split('.')[1]
		collect_single_HEADER_list.append(sample_NAME)
		header_list, column_dict = mothur_result_parser(each_file)
		if 'numsampled' not in collect_single_VALUE_dict:
			collect_single_VALUE_dict['numsampled'] = ['0']
			#collect_single_HEADER_list.append('numsampled')
		new_numsampled_length = len(column_dict['numsampled'])
		if new_numsampled_length > len(collect_single_VALUE_dict['numsampled']):
			collect_single_VALUE_dict['numsampled'] = column_dict['numsampled']
		collect_single_VALUE_dict[sample_NAME] = column_dict[prefix_NAME]
	#print collect_single_VALUE_dict
	# PLOTTING
	collector_curve_linechart_TRACE_objects_dict = {}
	
	reverse_DESIGN_dict = {}
	reverse_DESIGN_dict = design_dict_maker(design_file_PATH, 'reverse')
	
	for each_SAMPLE in collect_single_HEADER_list:
		collector_curve_linechart_TRACE_objects_dict[each_SAMPLE] = PLOTLY_GO.Box(
			#x=collect_single_VALUE_dict['numsampled'],
			y=collect_single_VALUE_dict[each_SAMPLE],
			name=each_SAMPLE,
			x=each_SAMPLE,
			hoverinfo='name',
			boxmean=True
		)
	collector_curve_linechart_LAYOUT_object = PLOTLY_GO.Layout(
		height=600,
		autosize=True,
		title='Rarefaction curve plot of ',
		showlegend=True,
		hovermode='closest',
		titlefont=dict(
			family='Avenir',
	...
	#print collector_curve_linechart_TRACE_objects_dict.values()
	collector_curve_linechart_FIGURE_objects = {}
	collector_curve_linechart_FIGURE_objects['shannon'] = PLOTLY_GO.Figure(data=collector_curve_linechart_TRACE_objects_dict.values(), layout=collector_curve_linechart_LAYOUT_object)
	# ###################################  PLOTLY PLOT CREATE
	plotly_html_string, plotly_script = plotly_html_maker('COLLECTOR_CURVE', 'LINE_CHART', collector_curve_linechart_FIGURE_objects, mode_bar=False)
	return (plotly_html_string, plotly_script)
...
def heatmap_generator(distance_matrix):
	...
	data = numpy.genfromtxt(distance_matrix, names=True, dtype=float, delimiter="\t")
	data_array = data.view((numpy.float, len(data.dtype.names)))
...
def heatmap_2_generator(sample_matrix, biom_matrix):
	# get data
	sample_data = numpy.genfromtxt(sample_matrix, names=True, dtype=float, delimiter="\t")
	sample_data_array = sample_data.view((numpy.float, len(sample_data.dtype.names)))
	...
	biom_data = numpy.genfromtxt(biom_matrix, names=True, dtype=float, delimiter="\t")
	biom_data_array = biom_data.view((numpy.float, len(biom_data.dtype.names)))
...
def mothur_collect_single(processors_COUNT, outputdir_PATH, stderr, stdout, run_pid, mothur_exec_PATH, shared_file_PATH, frequency_VALUE, method_NAME):
	# grab path of paired fastq files and save it into container
	space = ' '
	mothur_input_dictionary = {}
	...
	mothur_input_dictionary['command'] = command
	mothur_input_dictionary['mothur_exec_path'] = mothur_exec_PATH
	mothur_input_dictionary['processors'] = processors_COUNT
	mothur_input_dictionary['outputdir'] = outputdir_PATH
	mothur_input_dictionary['nohup_in'] = 'nohup'
	mothur_input_dictionary['nohup_out'] = '> ' + stdout + ' 2> ' + stderr + ' & echo $! > ' + run_pid
	parameter_list = []
	parameter_list.append('shared=' + shared_file_PATH)
	parameter_list.append(',' + space + 'freq=' + str(frequency_VALUE))
	parameter_list.append(',' + space + 'calc=' + str(method_NAME))
	parameter_list.append(',' + space + 'abund=5')
	mothur_input_dictionary['parameters'] = parameter_list
	make_file_object = mothur_process(mothur_input_dictionary)
	exec_dict = {}
	flag, exec_dict = make_file_object.execute_mothur_command()
...
def phylotype_to_shared_convert(phylotype_file_PATH, shared_file_PATH, taxlevel_NAME, prefix_NAME, outputdir_PATH):
	entity_list = []
	entity_list = parse_abundance_file(phylotype_file_PATH)
	dict_of_entities = {}
	dict_of_entities = construct_object(entity, entity_list)
	shared_file_generated_PATH = create_shared_table(dict_of_entities, phylotype_file_PATH, taxlevel_NAME, prefix_NAME, outputdir_PATH)
	os.rename(shared_file_generated_PATH, shared_file_PATH)
	return True
...
def match_shared_with_design(shared_file_PATH, design_file_PATH, control_file_PATH, new_shared_file_PATH, new_design_file_PATH, prefix_NAME, mothur_exec_PATH, processors_COUNT, outputdir_PATH):
	...
	if flag is False:
		...
		extension_list = ['.' + prefix_NAME + '.pick' + extension]
	...
	copy_file(design_file_PATH, new_design_file_PATH)
	return True


def design_to_control_convert(design_file_PATH, control_file_PATH):
	string = ''
...
def shared_dict_to_file_convert(shared_file_PATH, shared_HEADER_list, shared_CONTENT_dict, prefix_NAME):
	shared_file_CONTENT_string = ''
	shared_file_CONTENT_string = list_to_string(shared_HEADER_list) + '\n'
...
def shared_file_validator(shared_file_PATH, prefix_NAME, shared_file_VALIDATED_PATH):
	...
	if shared_file_DELIMITER not in ['\t']:
		print "Shared file delimiter: ", shared_file_DELIMITER, " is not standard, It Should be Tab delimited"
		report("Shared file delimiter: " + shared_file_DELIMITER + " is not standard, It Should be Tab delimited")
	...
	#print shared_file_HEADER_list
	shared_file_OBJECT = csv.DictReader(shared_file_HANDLE, shared_file_HEADER_list, delimiter=shared_file_DELIMITER)
	...
	#print shared_file_OBJECT_dict
	print "Shared file validation is in progress:"
	report("Shared file validation is in progress:")
	#shared_file_HEADER_list_length = len(shared_file_HEADER_list)
	shared_file_SAMPLE_length = len(shared_file_OBJECT_dict['label'])
	for each_HEAD in shared_file_HEADER_list:
		each_COLUMN_length = len(shared_file_OBJECT_dict[each_HEAD])
		if each_COLUMN_length != shared_file_SAMPLE_length:
			print "Shared file validation FAILED"
			error("Shared file validation FAILED")
			print "ABORTING!!!"
			error("ABORTING!!!")
			sys.exit(2)
		if None in shared_file_OBJECT_dict[each_HEAD] is True:
			print "Shared file validation FAILED"
			error("Shared file validation FAILED")
			print "ABORTING!!!"
			error("ABORTING!!!")
	...
	for each_HEAD in shared_file_HEADER_list:
		if each_HEAD.lower() in ['label', 'numotus', 'groups', 'group']:
			continue
		else:
	...
	if 'Group' in shared_file_HEADER_list:
		shared_file_NEW_HEADER_list.append('Group')
	elif 'Groups' in shared_file_HEADER_list:
	...
	flag = shared_dict_to_file_convert(shared_file_VALIDATED_PATH, shared_file_NEW_HEADER_list, shared_file_OBJECT_dict, prefix_NAME)
	if flag is True:
		pass
	print "Shared file validation PASSED!!!"
	report("Shared file validation PASSED!!!")
	return True
...
def design_file_validator(design_file_PATH, design_file_VALIDATED_PATH):
	...
	design_file_HEADER_list = []
	design_file_HEADER_list = design_file_HANDLE.readline().rstrip().split(design_file_DELIMITER)

	design_file_OBJECT = csv.DictReader(design_file_HANDLE, design_file_HEADER_list, delimiter=design_file_DELIMITER)
	design_file_OBJECT_dict = {}
	
	for each_OBJECT in design_file_OBJECT:
		for key, value in each_OBJECT.items():
			if key == value:
				continue
			elif key in design_file_OBJECT_dict:
				design_file_OBJECT_dict[key].append(value)
			else:
				design_file_OBJECT_dict[key] = [value]
	#print design_file_OBJECT_dict
	print "Design file validation is in progress:"
	report("Design file validation is in progress:")

	design_file_SAMPLE_length = len(design_file_OBJECT_dict[design_file_HEADER_list[0]])
	for each_HEAD in design_file_HEADER_list:
		each_COLUMN_length = len(design_file_OBJECT_dict[each_HEAD])
		if each_COLUMN_length != design_file_SAMPLE_length:
			print "Design file validation FAILED"
			error("Design file validation FAILED")
			print "ABORTING!!!"
			error("ABORTING!!!")
			sys.exit(2)
		if None in design_file_OBJECT_dict[each_HEAD] is True:
			print "Design file validation FAILED"
			error("Design file validation FAILED")
			print "ABORTING!!!"
			error("ABORTING!!!")
